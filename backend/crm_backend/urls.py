...
def serve_favicon(request):
    ...
    if os.path.exists(favicon_path):
        ...
        # Fallback to template favicon
        template_favicon = os.path.join(settings.BASE_DIR, 'templates', 'favicon.ico')
        if os.path.exists(template_favicon):
            with open(template_favicon, 'rb') as f:
                return HttpResponse(f.read(), content_type='image/x-icon')
    return HttpResponse(status=404)

urlpatterns = [
    # Favicon route (must be first)
